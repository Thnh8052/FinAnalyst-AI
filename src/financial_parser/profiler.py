...
VECTOR_CHART_PATTERN = re.compile(
    r"(?:stock performance graph|performance graph|cumulative total return|"
    r"total shareholder return|cumulative total shareholder return|"
    r"the following graph|the graph (?:below|above|shows)|"
    r"shareholder return graph|five-year cumulative total|5-year cumulative total|"
    r"biểu đồ|đồ thị)",
    re.IGNORECASE,
)


def _has_vector_chart(page: Any, text: str) -> bool:
    """Detect pages containing vector-drawn charts (Stock Performance graphs, trend lines)."""
    if not text:
        return False
    if not VECTOR_CHART_PATTERN.search(text):
        return False
    ...
    try:
        if hasattr(page, "get_drawings"):
            ...
            has_paths = any(
                any(item[0] in ("l", "c") for item in d.get("items", []))
                for d in drawings
            )
            return has_paths or len(drawings) >= 5
    except Exception:
        pass

    normalized = text.lower()
    return "performance graph" in normalized or "cumulative total" in normalized
...
def profile_page(page: Any, pdf_page: int, config: ParserConfig) -> PageProfile:
    ...
    has_vector_chart = _has_vector_chart(page, cleaned)
    reasons: List[str] = []
    ...
    elif max_coverage > config.max_image_coverage_for_native or total_coverage > 0.30:
        page_class = PageClass.HYBRID
        reasons.append("material_embedded_image_area")
    elif has_vector_chart:
        page_class = PageClass.HYBRID
        reasons.append("vector_chart_or_graph_detected")
    else:
