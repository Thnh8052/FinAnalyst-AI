"""
method3_heading_llm.py

Phương pháp 3 (Intermediate):
...
def run_method3_for_company(company_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for grp in all_groups:
        ...
        if total_words <= MAX_SECTION_WORDS:
            ...
            chunk_id = f"{ticker.lower()}_{fiscal_year}_m3_c{chunk_index:04d}"
            c = FinancialChunk(
                chunk_id=chunk_id,
                document_id=dir_name,
                ticker=ticker,
                fiscal_year=fiscal_year,
                chunk_method=ChunkMethod.METHOD3_HEADING_LLM.value,
            ...
            for sub_idx, st in enumerate(sub_texts, 1):
                ...
                chunk_id = f"{ticker.lower()}_{fiscal_year}_m3_c{chunk_index:04d}_p{sub_idx:02d}"
                c = FinancialChunk(
                    chunk_id=chunk_id,
                    document_id=dir_name,
                    ticker=ticker,
    ...
    # Luu output rieng biet vao method3_heading_llm
    output_company_dir = OUTPUT_CHUNKING_ROOT / dir_name / "method3_heading_llm"
    ...
    summary = {
        ...
        "ticker": ticker,
        "fiscal_year": fiscal_year,
        "method": ChunkMethod.METHOD3_HEADING_LLM.value,
...
def run_all_method3() -> Dict[str, Any]:
    print("=== BẮT ĐẦU CHẠY PHƯƠNG PHÁP 3: HEADING PRE-SPLIT + BATCH LLM MERGE + WORD FALLBACK ===")
    ...
    # Viết Master Report cho Phương pháp 3
    report_path = OUTPUT_CHUNKING_ROOT / "METHOD3_HEADING_LLM_EVALUATION_REPORT.md"
    report_lines = [
        "# Bao Cao Danh Gia Phuong Phap 3: Heading Pre-Split + Batch LLM Merge (Intermediate)",
    ...
    report_lines.extend([
        ...
        "---",
        "",
        "## 2. Nhận Xét Chuyên Môn Về Phương Pháp 3",
        "",
        ...
        "   - Tuy nhiên, vì Phương pháp 3 cắt dựa trên Heading Markdown chứ chưa có bộ nhận thức ma trận chuyên sâu như SectionStitcher/Table-Atomic của Phương pháp 2, một số bảng nằm xen giữa các đoạn văn dài bị fallback cắt trúng.",
        "",
        "3. **Ý nghĩa bước đệm:**",
        "   - Phương pháp 3 chứng minh rõ: **Khi có sự hỗ trợ của LLM ở bước gộp ngữ nghĩa, các chunk văn bản thuyết minh trở nên mạch lạc và tự nhiên hơn rất nhiều**.",
        "   - Đây là bước đệm để bước sang **Phương pháp 5 (Proposed Golden Hybrid)** kết hợp Semantic Grouping của LLM với Hard Structural Constraints của Code.",
        "",
    ])

    report_path.write_text("\n".join(report_lines), encoding="utf-8")
    print(f"\n Đã ghi Master Report cho Phương pháp 3 tại: {report_path}")
