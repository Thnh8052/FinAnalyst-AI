...
3. Method 2.5 (Baseline 3): Heading Pre-Split + Batch LLM Merge + Word Window Fallback
...
COMPANY_SPECS = [
    {"dir_name": "nvidia_2025_10k", "ticker": "NVDA", "name": "NVIDIA Corporation", "pages": 147},
    {"dir_name": "amd_10k_2025", "ticker": "AMD", "name": "Advanced Micro Devices, Inc.", "pages": 118},
    {"dir_name": "apple_2025_10k", "ticker": "AAPL", "name": "Apple Inc.", "pages": 88},
    {"dir_name": "intel_2025_10k", "ticker": "INTC", "name": "Intel Corporation", "pages": 160},
]
...
METHODS = [
    ...
    {
        "key": "method2_5_heading_llm",
        "name": "Method 2.5: Heading Pre-Split + LLM Merge",
        "short": "Method 2.5 (Heading+LLM)",
        "cost": "~$0.02 (DeepSeek)",
        "dual_rep": "Không",
        "stitcher": "Không",
    },
...
def generate_master_report() -> str:
    ...
    report_lines.append("**Tập dữ liệu thử nghiệm:** 4 tập đoàn công nghệ hàng đầu (NVIDIA, AMD, Apple, Intel) — 513 trang báo cáo 10-K tài khóa 2025.")
    report_lines.append(f"**Ngày hoàn tất đánh giá:** 19/09/2026.\n")

    report_lines.append("---")
    report_lines.append("## 1. Bảng So Sánh Hiệu Năng Tổng Thể Giữa 5 Phương Pháp (Cross-Method Benchmark)\n")
    report_lines.append("| Tiêu Chí Đánh Giá | Method 1: Naive Fixed-Size | Method 2: Deterministic Structure | Method 2.5: Heading + LLM Merge | Method 4: LLM Boundary Tagging | Method 5: Proposed Golden Hybrid |")
    report_lines.append("| :--- | :---: | :---: | :---: | :---: | :---: |")

    m1_tot = method_totals["baseline_fixed_size"]
    m2_tot = method_totals["baseline_deterministic_structure"]
    m25_tot = method_totals["method2_5_heading_llm"]
    ...
    report_lines.append(f"| **Bảo toàn Bảng Đa Trang (Stitched)** | Không | Có (14 bảng ghép thành công) | Không | Không | Có (SectionStitcher tích hợp) |")
    ...
    report_lines.append(f"| **Chi phí gọi LLM (API Cost)** | $0.00 | $0.00 | ~$0.02 (DeepSeek) | ~$0.03 (DeepSeek) | ~$0.06 (DeepSeek) |")
    ...
    report_lines.append("\n### 3.3 Method 2.5 (Heading Pre-Split + LLM Merge): Cải Thiện Nhưng Còn Lỗ Hổng (9.62% Vỡ Bảng)")
    ...
    report_lines.append("- **Kết quả:** Tỷ lệ vỡ bảng chỉ còn **2.76% (58 chunks)** — tốt hơn nhiều so với Method 1 (20.88%) và Method 2.5 (9.62%). LLM có khả năng nhận biết ranh giới chủ đề rất tốt.")
    ...
    report_lines.append("- `output_chunking/<company>/method2_5_heading_llm/chunks.jsonl`")
